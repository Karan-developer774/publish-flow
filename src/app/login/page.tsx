...
export default function LoginPage() {
  ...
  const handleLogin = () => {
    ...
    router.push(ROLE_META[selected].redirect);
  };
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {getAllRoles().map((role) => {
            ...
            return (
              <Card
                key={role}
                className={`cursor-pointer transition-all ${isSelected ? "ring-2 ring-primary border-primary" : "hover:border-muted-foreground/30"}`}
