...
const AdminPage = () => {
  ...
  const handlePublish = (id: string) => {
    ...
    if (res.success) { toast.success('Page published'); refreshPages(); }
    else toast.error(res.error);
  };

  const handleUnpublish = (id: string) => {
    const res = api.unpublishPage(role, id);
    if (res.success) { toast.success('Page unpublished'); refreshPages(); }
    else toast.error(res.error);
  };

  const handleDelete = (id: string) => {
    const res = api.deletePageById(role, id);
    if (res.success) { toast.success('Page deleted'); refreshPages(); }
    else toast.error(res.error);
  };
    ...
    <div className="min-h-screen p-6 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Logged in as <Badge variant="outline">{role}</Badge></p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => navigate('/editor')}>Editor View</Button>
          <Button variant="outline" size="sm" onClick={() => navigate('/login')}>Switch Role</Button>
        </div>
      </div>
          ...
          <Card key={page.id}>
            <CardContent className="flex items-center justify-between py-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">/{page.slug} • Created by {page.createdBy}</p>
              </div>
